...
const endpoint = '/relaybounces';

class RelayBounces extends AbstractTool {
  constructor() {
  ...
  async run(params: any, body: any, query: any): Promise<PugTemplate> {
    ...
    return {
      type: 'pug',
      template: 'inxmail/relaybounces',
      payload: {
        title: `Relay-Bounces (${begin.toLocaleString('de-DE', { timeZone: 'Europe/Berlin' })} - ${end.toLocaleString('de-DE', { timeZone: 'Europe/Berlin' })})`,
        results: sortedResults,
        hasContent,
        prevLink: prevLink && `/inxmail/relaybounces/${prevLink}`,
        nextLink: nextLink && `/inxmail/relaybounces/${nextLink}`,
      }
    };
  }
}

export default RelayBounces;
