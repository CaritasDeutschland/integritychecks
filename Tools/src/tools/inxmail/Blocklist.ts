...
class Blocklist extends AbstractTool {
  constructor() {
    super('GET', [
      {
        name: 'email',
  ...
  async run(params: any, body: any, query: any): Promise<PugTemplate> {
    ...
    return {
      type: 'pug',
      template: 'inxmail/blocklist',
      payload: {
        title: `Blocklist (${begin.toLocaleString('de-DE', { timeZone: 'Europe/Berlin' })} - ${end.toLocaleString('de-DE', { timeZone: 'Europe/Berlin' })})`,
        results: sortedResults,
        hasContent,
        prevLink: prevLink && `/inxmail/blocklist/${prevLink}`,
        nextLink: nextLink && `/inxmail/blocklist/${nextLink}`,
      }
    };
  }
}
